...
from gymnasium_envs.det_env import AprilTagDet_Depth
...
class KitchenSingleTool(Task):
    ...
    def get_obs(self) -> np.ndarray:
        ...
        grab_obj_pos = _normalization(grab_obj_pos, self.reset_goal_max_pos, self.reset_goal_min_pos, range_max=self.norm_max, range_min=self.norm_min)
        # grab_obj_pos = _normalization(grab_obj_pos, self.ee_high, self.ee_low, range_max=self.norm_max, range_min=self.norm_min)
    ...
    def is_success(
        ...
        if self.curr_skill is self.specified_skills[1]:  # flip skill, the done need rotation
            ...
            # print('success reward for pouring:', z_dis, xy_dis)
...
class KitchenSingleTool_UR5e_real(Task):
    ...
    def __init__(self,
        ...
        self.det_obj_name = 'banana'
        self.board_pos = np.zeros(3)
        self.obj_pos = np.zeros(3)
        self.det_flag = False
        det_thread = threading.Thread(target=self._target_obj_det)
        det_thread.daemon = True
        det_thread.start()

        # input()  # test for detection part and switching the target detected object
        #
        # self.det_obj_name = 'apple'
        #
        # input()

    def _target_obj_det(self):
        '''Object and board detection in the real world'''
        tag_obj_det = AprilTagDet_Depth(rootid=5, objid=6, yolo_det=True, render=False)
        while True:
            self.board_pos, self.obj_pos, self.det_flag = tag_obj_det.robot2tag(object_name=self.det_obj_name)
            # print(self.board_pos, self.obj_pos)
    ...
    def get_obs(self) -> np.ndarray:
        ...
        # !!! real world ---------------------------
        grab_obj_quat = np.array([0.065641, -0.084931, 0.016664, 0.99408]) # test for reach
        grab_obj_pos = np.copy(self.goal)
        grab_obj_pos[-1] += 0.1
    ...
    def reset(self, skill_index):
        ...
        # reloading mj xml file
        # chose new env
        self.task_id = 0 # TODO: hard code for choosing task in dictionary
        self.task_chain = self.config['task_chain']['Mix_Fruit'] # TODO: hard code for choosing task in dictionary
        task_len = self.task_chain
        # TODO: the chosen can be set in the run function part, >>>>> trying reach skill first
        #   the skill_index is need used in the real world now, can modify the core function to use it
        # -------------------------------------

        self.curr_skill = 'reach'
        self.goal_name = 'apple'
        if self.goal_name == 'board':
            self.goal = np.copy(self.board_pos)
        else:
            self.det_obj_name = self.goal_name # could be changed by the task, such as banana, orange, cup, knife, the board would be detected all the time
            while True:
                time.sleep(0.1)
                if self.det_flag is True:
                    self.goal = np.copy(self.obj_pos)
                    self.det_flag = False
                    print(self.obj_pos)
                    break
